...
let g₀ : graph := naive_aevb a x_data,
    g_aevb : graph := reparam (integrate_kl g₀),
    fdict : env := mk_input_dict ws x_data g₀ in
...
=
E (graph.to_dist (λ m, backprop g_aevb^.costs g_aevb^.nodes m g_aevb^.targets) fdict g_aevb^.nodes)
  (λ dict, dvec.get tgt.2 dict idx) :=

begin
whnf_target, intros tgt idx H_at_idx,
simp only [@aevb_transformations_sound a ws x_data tgt idx H_at_idx],
apply backprop_correct_on_aevb,
rw [naive_aevb_as_graph] at H_at_idx, rw [naive_aevb_as_graph],
exact H_at_idx
end
