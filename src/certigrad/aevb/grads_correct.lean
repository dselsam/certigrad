...
do H_at_idx ← get_local `H_at_idx,
   ...
   to_expr ```(backprop_correct (fdict a ws x_data) (g a x_data)^.targets H_tgts_ss_inputs H_at_idx H_wf H_gs_exist H_gint H_can_diff) >>= exact

meta def prove_aevb_ok : tactic unit :=
do -- introduce hypotheses
   [tgt, idx, H_at_idx] ← intros | failed,
